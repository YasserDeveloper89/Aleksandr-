...
# Intentar elegir una voz más suave o infantil (ajústalo según tu sistema)
for voice in voices:
...
def bienvenida():
    st.image("logo_aleksandr.png", width=200)
    ...
    with col1:
        ...
        if st.button("Comprobar suma"):
            if r1 == 8:
                ...
                st.error("Ups, intenta otra vez.")
                narrar("Ups, intenta otra vez.")
...
def modulo_ahorro():
    ...
    r = st.number_input("Respuesta ahorro:", min_value=0, step=1, key="ahorro")
...
# ----------- Navegación -------------
def elegir_modulo():
    modulo = st.sidebar.selectbox("Elige un módulo:",
